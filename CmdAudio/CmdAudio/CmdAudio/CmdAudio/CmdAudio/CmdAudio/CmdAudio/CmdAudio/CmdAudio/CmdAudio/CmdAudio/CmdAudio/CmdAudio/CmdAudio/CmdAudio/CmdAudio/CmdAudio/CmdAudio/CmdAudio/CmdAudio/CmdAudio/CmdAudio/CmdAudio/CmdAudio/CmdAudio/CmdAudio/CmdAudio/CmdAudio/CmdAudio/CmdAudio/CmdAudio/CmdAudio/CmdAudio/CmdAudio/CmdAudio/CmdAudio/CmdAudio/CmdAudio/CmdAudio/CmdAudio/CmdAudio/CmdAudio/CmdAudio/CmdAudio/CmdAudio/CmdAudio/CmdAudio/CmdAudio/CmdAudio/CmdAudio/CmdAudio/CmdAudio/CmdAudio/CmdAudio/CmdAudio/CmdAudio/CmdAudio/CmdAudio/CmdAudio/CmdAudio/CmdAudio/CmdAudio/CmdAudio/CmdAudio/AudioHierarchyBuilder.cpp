...
public:
    ...
    virtual void enterStat(CmdAudioParser::StatContext * ctx) override
    {
        ...
        BaseCountedPtr<GeneratorBase> statementGenerator = new GeneratorStatement( ProcessExpression( expression ) );
        ...
        else if ( expression->prefixexp() )
        {
            ...
            if ( prefixExp->varOrExp() )
            {
                ...
                if (varOrExp->var())
                {
                    ...
                    {
                        if ( m_statementLocalsUsage.find(varName)->second > 0)
                        {
                            generator = new GeneratorRef( foundVarIter->second );
                        }
                        else
                        {
